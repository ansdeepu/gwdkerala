                            ...
                            <FormField 
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        {isLinkedToRemittance && <FormDescription className="text-xs">This is auto-managed by a 'Revenue Head' remittance.</FormDescription>}
                                        <FormMessage />
                                    </FormItem>
                                )}
                            />
...
export default function LoggingPumpingTestDataEntryFormComponent({ fileNoToEdit, initialData, userRole, workTypeContext, returnPath, pageToReturnTo, isFormDisabled = false, allLsgConstituencyMaps, allStaffMembers }: DataEntryFormProps) {
  ...
  const handleDialogConfirm = (data: any) => {
    ...
    if (type === 'application') {
        ...
        let paymentIndex = -1;
        if(originalData?.id) {
            paymentIndex = paymentFields.findIndex(p => p.remittanceId === originalData.id);
        }

        // Logic for handling "Revenue Head"
        if (remittanceData.remittedAccount === 'Revenue Head') {
            ...
            if (originalData.index !== undefined) { // EDIT
                // If a payment was linked, update it. Otherwise, create one.
                if (paymentIndex !== -1) {
                    updatePayment(paymentIndex, newPaymentEntry);
                } else {
        ...
        } else { // Not a "Revenue Head" remittance
            if (originalData.index !== undefined) { // EDIT
                // If it WAS a Revenue Head before, delete the old payment.
                if (paymentIndex !== -1) {
        ...
        if (originalData.index !== undefined) updateReappropriation(originalData.index, data); else appendReappropriation(data);
  ...
  const handleDeleteItem = async () => {
    ...
    if (type === 'remittance') {
        ...
        if (remittanceToDelete.remittedAccount === 'Revenue Head') {
