...
const SiteDialogContent = ({ initialData, onConfirm, onCancel, isReadOnly, isSupervisor, allLsgConstituencyMaps, allStaffMembers, workTypeContext }: { initialData: any, onConfirm: (data: any) => void, onCancel: () => void, isReadOnly: boolean, isSupervisor: boolean, allLsgConstituencyMaps: any[], allStaffMembers: StaffMember[], workTypeContext: string | null }) => {
    ...
    const workStatusOptions = LOGGING_PUMPING_TEST_WORK_STATUS_OPTIONS;
    const purposeOptions = LOGGING_PUMPING_TEST_PURPOSE_OPTIONS;
  ...
  const handleDialogConfirm = (data: any) => {
    ...
    if (type === 'application') {
        ...
        if (finalRemittanceData.remittedAccount === 'Revenue Head' && finalRemittanceData.amountRemitted && finalRemittanceData.amountRemitted > 0) {
            ...
            toast({ title: "Payment Details Synced", description: "Payment record updated for Revenue Head remittance." });
        }
