...
export default function DataEntryFormComponent({ fileNoToEdit, initialData, supervisorList, userRole, workTypeContext, returnPath, pageToReturnTo, isFormDisabled = false }: DataEntryFormProps) {
  ...
  const { fields: remittanceFields, append: appendRemittance, remove: removeRemittance, update: updateRemittance, replace: replaceRemittance } = useFieldArray({ control, name: "remittanceDetails" });
  ...
  const sortedCombinedReappropriations = useMemo(() => {
   ...
   useEffect(() => {
        ...
        // To avoid infinite loops, only update if there's an actual change.
