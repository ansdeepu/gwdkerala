...
export default function PrivateDepositWorksPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('private_works_search');
    if (saved) setSearchTerm(saved);
  }, []);

  useEffect(() => {
    localStorage.setItem('private_works_search', searchTerm);
  }, [searchTerm]);
  ...
  const filteredEntries = useMemo(() => {
    if (!searchTerm) return privateDepositWorkEntries;
